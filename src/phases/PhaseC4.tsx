// PhaseC4 — MNIST 도전 (visible 커리큘럼 마지막 페이즈)
...
function Workbench({ samples, meta }: { samples: Sample[]; meta: { num: string; title: string } }) {
  ...
  const runTraining = async (extra: TrainSample[], beforeAcc: number | null) => {
    ...
    setTraining(false);
    if (testAcc > 0.7) markCompleted('c4');
  };
  ...
  return (
    <article>
      <div className="text-xs font-mono text-muted">PHASE {meta.num}</div>
      <h1>{meta.title}</h1>
      {/* PLAN ## 10-1 #7 도입부 본문 그대로 (B5 → C4 다리). "같은 구조" 표현 금지. */}
      <p className="mt-2 text-sm leading-relaxed">
        B5에서는 출력 뉴런 3개로 동그라미·세모·네모를 갈랐죠. MNIST는 입력이 28×28로 훨씬 크고, 출력 뉴런을 10개로 늘려 0~9 숫자를 가릅니다.
        softmax로 확률을 만든다는 출력층의 원리는 같지만, 입력이 커진 만큼 C3에서 본 것처럼 은닉층 크기도 함께 키워야 해요.
      </p>
